...
export const Home = (props) => {
	...
	<div className="overflow-scroll d-flex container " style={{width: "1500px"}}>
		...
		{store.personajes.map((item, index)=> <Card1 name={item.name} gender={item.gender} hair={item.hair_color} eye={item.eye_color} id={index+1} key={index}/>)}
	...
	{store.planetas.map((item, index)=> <Card2 planetname={item.name} population={item.population} terrain={item.terrain} id={index+1} key={index}/>)}
